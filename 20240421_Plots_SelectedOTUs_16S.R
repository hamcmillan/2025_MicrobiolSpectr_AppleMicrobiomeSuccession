...
plot_selected_otus <- function(data, title, title2, otu_subset, physeq, label_separater, file_destination) {
  ...
  rel_data <- data
  rel_data[2:42] <- lapply(data[2:42], function(x) (x/sum(x))*100)
...
# 16S pulp - shared all timepoints
data <- read.csv("DESeq2_Results/pulp/normalized_data_pulp.csv")
title <- "16S_Gala_Pulp_OTUs_Shared_all_timepoints"
title2 <- "16S_Gala_Pulp_OTUs_Shared_all_timepoints_separated"
otu_subset <- shared_all_pulp
physeq <- readRDS("16S.physeq_filtered.rds")
label_separater <- "\\.g\\.p"
file_destination <- "SelectedOTU_Plots"

plot_selected_otus(data, title, title2, otu_subset, physeq, label_separater, file_destination)

# 16S pulp - shared fb and fm2
data <- read.csv("DESeq2_Results/pulp/normalized_data_pulp.csv")
title <- "16S_Gala_Pulp_OTUs_Shared_fb_and_fm2"
title2 <- "16S_Gala_Pulp_OTUs_Shared_fb_and_fm2_separated"
...
# 16S skin shared all timepoints
data <- read.csv("DESeq2_Results/skin/normalized_data_skin.csv")
title <- "16S_Gala_Skin_OTUs_Shared_all_timepoints"
title2 <- "16S_Gala_Skin_OTUs_Shared_all_timepoints_separated"
...
# 16S skin shared fb and fm2
data <- read.csv("DESeq2_Results/skin/normalized_data_skin.csv")
title <- "16S_Gala_Skin_OTUs_Shared_fb_and_fm2"
title2 <- "16S_Gala_Skin_OTUs_Shared_fb_and_fm2_separated"
...
# 16S pulp
pdf("pulp_16S_venndiagram.pdf", height = 6, width = 8)
...
# 16S skin
pdf("skin_16S_venndiagram.pdf", height = 6, width = 8)
...
filename <- "VennDiagrams/16S_pulp_fb_fm2.pdf"
...
color_list <- c("#0673B3", "#F1E544", "#989898", "#D46127")
filename <- "VennDiagrams/16S_skin_fb__fl2_fd2_fm2.pdf"
plot_venn_4(named_list, color_list, filename)
