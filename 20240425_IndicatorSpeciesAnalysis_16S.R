...
indicator_species_analysis <- function(input_file, subset_name, output_dir) {
  ...
  rel_data[2:42] <- lapply(norm_counts[2:42], function(x) (x/sum(x))*100)
...
# pulp
input_file <- "DESeq2_Results/pulp/normalized_data_pulp.csv"
subset_name <- "gala_pulp"
output_dir <- "IndicatorSpecies_16S"

indicator_species_analysis(input_file, subset_name, output_dir)

# skin
input_file <- "DESeq2_Results/skin/normalized_data_skin.csv"
